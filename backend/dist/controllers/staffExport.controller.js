...
exports.exportConcessionsExcel = exportConcessionsExcel;
...
function addMeta(sheet, start, end, staffId) {
    sheet.insertRow(1, [
        `Exported By Staff ID: ${staffId} | Range: ${start.toLocaleDateString("en-IN")} to ${end.toLocaleDateString("en-IN")} | Generated: ${new Date().toLocaleString("en-IN")}`,
    ]);
    sheet.mergeCells("A1:N1");
    sheet.getRow(1).font = { bold: true };
}
async function exportConcessionsExcel(req, res) {
    try {
        const { range, from, to } = req.query;
        const { start, end } = getDateRange(range, from, to);
        const staffId = req.user.sub;
        const approved = await client_1.prisma.concessionApplication.findMany({
            where: {
        ...
        const rejected = await client_1.prisma.concessionApplication.findMany({
            where: {
                approvedByStaffId: staffId,
                status: "REJECTED",
                rejectedAt: {
        ...
        res.setHeader("Content-Disposition", "attachment; filename=concessions-export.xlsx");
