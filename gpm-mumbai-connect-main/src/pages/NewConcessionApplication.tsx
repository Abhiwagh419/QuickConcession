...
const NewConcessionApplication = () => {
  ...
  const fromStations = formData.fromLine ? stationsByLine[formData.fromLine] || [] : [];
  const toStations = formData.toLine ? stationsByLine[formData.toLine] || [] : [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {setIsSubmitting(true);
    ...
    try {
      ...
      toast({
        ...
        description: "Your railway concession application has been submitted successfully.",
      });
      ...
      <main className="container mx-auto px-4 py-6 max-w-3xl">
        ...
        <Card className="border shadow-sm mb-6 bg-secondary/30">
          <CardContent className="pt-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                <p className="text-xs text-muted-foreground uppercase">Enrollment No</p>
              ...
              <div>
                <p className="text-xs text-muted-foreground uppercase">Year/Semester</p>
                <p className="font-medium">{student.year} / {student.sem}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border shadow-sm">
          <CardHeader className="pb-3 border-b bg-muted/30">
            <CardTitle className="text-lg font-heading">Concession Details</CardTitle>
          </CardHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="travelClass">Class <span className="text-destructive">*</span></Label>
                  <Select value={formData.travelClass} onValueChange={(v) => handleChange("travelClass", v)}>
                    <SelectTrigger id="travelClass" className={errors.travelClass ? "border-destructive" : ""}>
  ...
  {errors.startDate && (
    <p className="text-xs text-destructive">{errors.startDate}</p>
              ...
              <div className="space-y-4">
                <h3 className="font-medium text-foreground border-b pb-2">Departure Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="fromLine">From Line <span className="text-destructive">*</span></Label>
                    <Select value={formData.fromLine} onValueChange={(v) => handleChange("fromLine", v)}>
                      <SelectTrigger id="fromLine" className={errors.fromLine ? "border-destructive" : ""}>
                    ...
                    {errors.fromLine && <p className="text-xs text-destructive">{errors.fromLine}</p>}
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="fromStation">From Station <span className="text-destructive">*</span></Label>
                    <Select 
                      value={formData.fromStation} 
                      onValueChange={(v) => handleChange("fromStation", v)}
                      disabled={!formData.fromLine}
                    >
                      <SelectTrigger id="fromStation" className={errors.fromStation ? "border-destructive" : ""}>
                        <SelectValue placeholder={formData.fromLine ? "Select station" : "Select line first"} />
                      </SelectTrigger>
              ...
              {/* To Section */}
              <div className="space-y-4">
                <h3 className="font-medium text-foreground border-b pb-2">Arrival Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="toLine">To Line <span className="text-destructive">*</span></Label>
                    <Select value={formData.toLine} onValueChange={(v) => handleChange("toLine", v)}>
                      <SelectTrigger id="toLine" className={errors.toLine ? "border-destructive" : ""}>
                        <SelectValue placeholder="Select railway line" />
                      </SelectTrigger>
                      <SelectContent className="bg-card">
                        {railwayLines.map((line) => (
                    ...
                    {errors.toLine && <p className="text-xs text-destructive">{errors.toLine}</p>}
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="toStation">To Station <span className="text-destructive">*</span></Label>
                    <Select 
                      value={formData.toStation} 
                      onValueChange={(v) => handleChange("toStation", v)}
                      disabled={!formData.toLine}
                    >
                      <SelectTrigger id="toStation" className={errors.toStation ? "border-destructive" : ""}>
                        <SelectValue placeholder={formData.toLine ? "Select station" : "Select line first"} />
                      </SelectTrigger>
                      <SelectContent className="bg-card max-h-60">
              ...
              {formData.fromStation && formData.toStation && (
                <div className="p-4 bg-secondary/50 rounded-lg border">
                  <p className="text-sm font-medium text-foreground mb-1">Route Summary:</p>
                  <p className="text-sm text-muted-foreground">
                    {formData.fromStation} ({getLineName(formData.fromLine)}) → {formData.toStation} ({getLineName(formData.toLine)})
                  </p>
              ...
              <div className="p-4 bg-warning/10 border border-warning/30 rounded-lg flex gap-3">
                ...
                <p className="text-sm text-foreground">
                  <strong>Important:</strong> Applications may be rejected if incorrect information is provided. 
